...
def add_common_opening(doc: Document, lab: LabReport):
    ...
    para(
        ...
        "разработать алгоритм функционирования устройства, описать его на языке VHDL, "
    ...
    para(
        doc,
        "Лабораторная работа выполняется в среде Quartus II 9.1. Описание устройства выполнено "
        "на синтезируемом подмножестве VHDL с использованием библиотеки IEEE std_logic_1164 и, "
        "где это требуется, numeric_std. Разрядность основных трактов данных принята равной 16 бит, "
        "что соответствует принятому варианту курсового проекта.",
    )
    para(
        doc,
        "Для удобства просмотра схемы в RTL Viewer верхним уровнем проекта является модуль "
        f"{lab.top}. Он представляет собой тонкую обертку над рабочим структурным модулем: "
        "алгоритм устройства не изменяется, но внешние пины имеют развернутые имена, по которым "
        "понятно назначение команды, операндов, флагов, линий памяти и диагностических сигналов.",
    )
...
def add_execution(doc: Document, lab: LabReport):
    ...
    para(
        doc,
        "Структура проекта выбрана модульной: каждый функциональный узел вынесен в отдельную VHDL-entity, "
        "а верхний модуль соединяет эти узлы сигналами данных и управления. Такой способ удобен для проверки, "
        "так как отдельные блоки можно анализировать в RTL Viewer и независимо покрывать моделированием.",
    )
    ...
    para(
        doc,
        "В таблице приведены основные точки наблюдения верхнего уровня. Эти же имена отображаются "
        "на схеме RTL Viewer после компиляции проекта и используются при ручном снятии временных диаграмм.",
    )
    ...
    subheading(doc, "4.4 Реализация на языке VHDL")
    for paragraph in lab.implementation:
        para(doc, paragraph)
    add_table(doc, ["Файл", "Назначение"], lab.module_rows, font_size=9.5)
    para(
        doc,
        f"Файл проекта Quartus: {lab.folder}/quartus/{lab.qpf}. Верхний уровень проекта: {lab.top}. "
        "После открытия проекта необходимо выполнить Processing -> Start Compilation, затем открыть "
        "Tools -> Netlist Viewers -> RTL Viewer для просмотра схемы.",
    )
    ...
    para(
        doc,
        "Компиляция проекта выполняется в Quartus II 9.1. VHDL-описание не использует вручную "
